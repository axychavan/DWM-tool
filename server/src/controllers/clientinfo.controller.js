const clientinfoModel = require('../models/clientinfo.model');

//get all records
exports.getRecords = (req, res)=>{
    //console.log('List of all the records');
    clientinfoModel.getAllRecords((err, records)=>{
...
exports.getRecordByID = (req, res)=>{
    ...
    clientinfoModel.getRecordByID(req.params.clientid, (err, record)=>{
        if(err)
        res.send(err);
...
exports.createNewRecord = (req, res)=>{ 
    const recordReqdata = new clientinfoModel(req.body);
    ...
    if(req.body.constructor === Object && Object.keys(req).length === 0){
        ...
        clientinfoModel.createRecord(recordReqdata, (err, records)=>{
            if(err)
            res.send(err);
...
exports.updateRecord = (req, res)=>{
    ...
    if(req.body.constructor === Object && Object.keys(req).length === 0){
        ...
        console.log('valid data');
        clientinfoModel.updateRecord(req.params.id, recordReqdata, (err, records)=>{
            if(err)
            res.send(err);
...
exports.deleteRecord = (req, res)=>{
    clientinfoModel.deleteRecord(req.params.id, (err, records)=>{
        if(err)
        res.send(err);
